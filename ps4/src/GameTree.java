...
public class GameTree {
    ...
    int findValue() {
        // TODO: Implement this
        return findValueHelper(Player.ONE, this.root);
    }

    public int findValueHelper(Player player, TreeNode node) {
        if (node.leaf) {
            return node.value;
        }
        if(node == null) {
        ...
        if (player == Player.ONE) {
            ...
            for (int i = 0; i < node.numChildren; i++) {
                int leafValue = findValueHelper(other(player), node.children[i]);
                largest = Math.max(leafValue, largest);
            }
        } else {
            largest = Integer.MAX_VALUE;
            for (int i = 0; i < node.numChildren; i++) {
                int leafValue = findValueHelper(other(player), node.children[i]);
                largest = Math.min(leafValue, largest);
            }
        }
        node.value = largest;
        return largest;
